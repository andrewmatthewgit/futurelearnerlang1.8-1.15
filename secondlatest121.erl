-module(secondlatest121).
-export([double/1,product/1,mult/2,maxlist/1,areaold/3,square/1,freq/2,freq2/1,even/1,take/2,doublelist/1,treble/1,mode/1,hypot/2,encl/1,perimtriangle/2,maxThree/3,howManyEqual/3,bits/1,fib/1,pieces/1,area/1,perim/1]).




% take/2 to extract requested number of items from a list

% picks out required part of the list except if requirment is 0 when result is empty list

take(0,_Xs) -> [];
take(_No,[]) -> [];

take(No,[X|Xs]) when No > 0
                ->      
                [X|take(No-1,Xs)].
